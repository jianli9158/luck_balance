...
func balanceBnbEth() {
	zero := big.NewInt(0)
	for i := 0; i < 18; i++ {
		fmt.Println("binanceMainnet: ", binanceMainnetList[i], i)
		fmt.Println("ethMainnet: ", etherMainnetList[i], i)
		bclient, err := ethclient.Dial(etherMainnetList[i])
		if err != nil {
			log.Println(err)
			if i == 17 {
				i = -1
		...
		eclient, err := ethclient.Dial(etherMainnetList[i])
		if err != nil {
			log.Println(err)
			if i == 17 {
				i = -1
		...
		for true {
			...
			// get eth balance1
			ethbalance, err := eclient.BalanceAt(context.Background(), account.Address, nil)
			if err != nil {
				log.Println(err)
				if i == 17 {
					i = -1
			...
			if ethbalance.Cmp(zero) == 1 {
				log.Println("牛逼!!")
				CreateHisi(fmt.Sprintf("地址:%s ---助记词语:%s ---ethBalance=%s!!!!\n", account.Address, mnemonic, ethbalance.String()))
			}
			// get bnb balance1
			bnbBalance, err := bclient.BalanceAt(context.Background(), account.Address, nil)
			if err != nil {
				log.Println(err)
				if i == 17 {
					i = -1
			...
			if bnbBalance.Cmp(zero) == 1 {
				log.Println("牛逼!!")
				CreateHisi(fmt.Sprintf("地址:%s 助记词语:%s --------bnbBalance=%s!!!!\n", account.Address, mnemonic, bnbBalance.String()))
			}
			// get wallet2
			...
			// get eth balance2
			ethbalance1, err := eclient.BalanceAt(context.Background(), account.Address, nil)
			if err != nil {
				log.Println(err)
				if i == 2 {
					i = -1
			...
			if ethbalance1.Cmp(zero) == 1 {
				log.Println("牛逼!!")
				CreateHisi(fmt.Sprintf("地址:%s 助记词语:%s --------ethbalance1=%s!!!!\n", account.Address, mnemonic, ethbalance1.String()))
			}

			// get bnb balance2
			bnbBalance1, err := bclient.BalanceAt(context.Background(), account.Address, nil)
			if err != nil {
				log.Println(err)
				if i == 12 {
					i = -1
			...
			if bnbBalance1.Cmp(zero) == 1 {
				log.Println("牛逼!!")
				CreateHisi(fmt.Sprintf("地址:%s 助记词语:%s --------bnbBalance1=%s!!!!\n", account.Address, mnemonic, bnbBalance1.String()))
			}
		}
